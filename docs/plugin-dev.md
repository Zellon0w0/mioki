...
> [!IMPORTANT]
> **插件优先实现：** 新功能、功能修复、WebUI 扩展、配置管理、热重载适配等需求，应尽量在对应插件目录内完成，避免修改 `packages/mioki` 核心包。只有当需求必须新增或调整框架级公共能力，且无法通过插件 API、服务注册、路由注册、配置文件或清理钩子实现时，才考虑修改核心包；此类改动必须明确说明原因、影响范围以及是否需要同步更新部署端的 `mioki` 版本。

> [!IMPORTANT]
> **WebUI 视觉一致性：** 凡涉及 WebUI 页面、组件或交互设计的插件，应以 [Material Design 3](https://m3.material.io/) 为设计基准，并与现有 `webui` 插件、`白名单管理` 插件保持一致的视觉语言。优先复用或对齐它们的设计令牌、颜色、表面层级、圆角、按钮、表单控件、状态反馈、间距与响应式布局；避免引入与 Material Design 3 风格冲突的装饰性背景、过度渐变或独立视觉体系。

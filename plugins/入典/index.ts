    ...
    ctx.handle('message.group', async (e) => {
      ...
      if (text === '入典') {
        ...
        try {
          ...
          imgPath = await generateRudianImage(content, senderName, avatarUrl)

          await ctx.bot.sendGroupMsg(e.group_id, [ctx.segment.image(`file://${imgPath}`)])
        } catch (err) {
